    ...
    public static List<DiffProperty> compareData(Map<String, Object> map1, Map<String, Object> map2) {
        ...
        for (String key : keys) {
            if (!map1.containsKey(key)) {
                diff.add(new DiffProperty(key, map2.get(key), DiffType.ADDED));
            } else if (!map2.containsKey(key)) {
                diff.add(new DiffProperty(key, map1.get(key), DiffType.REMOVED));
            } else {
                Object value1 = map1.get(key);
                Object value2 = map2.get(key);
                if (value1.equals(value2)) {
                    diff.add(new DiffProperty(key, value1, DiffType.UNCHANGED));
                } else {
